...
export const evalES = (script: string, isGlobal = false): Promise<string> => {
  return new Promise(function (resolve, reject) {
    ...
    // Errors come back as a string marker, never alert(e): an alert() opens a
    // modal dialog in AE, and while ANY modal is up every later evalScript
    // fails with "Cannot run a script while a modal dialog is waiting for
    // response" — with periodic evals that cascades into an alert storm.
    csi.evalScript(
      "try{" + fullString + "}catch(e){'__evalES_error__: '+String(e)}",
      (res: string) => {
        resolve(res);
      }
    );
